const dummyData = [
    {
      "confidence": 99.99,
      "crop_name": "Cotton",
      "disease": "Bacterial Blight",
      "crop_id": "b0f6be504c69440897464389f9374a81",
      "causes": null,
      "chemical_control": null,
      "symptoms": "Bacterial blight starts out as angular, waxy and water-soaked leaf spot with a red to brown border on leaves, stems and bolls. The angular appearance is due to the restriction of the lesions by fine veins on the cotton leaf. In some cases, the spots on the leaf blade may spread along the major leaf veins. As disease progresses, these lesions gradually turn into brown, necrotic areas. The infection of stems results in black cankers that grow around the vascular tissues and girdle them, causing the portions above the canker to die and premature defoliation of the plant. A white waxy crust containing the bacterium may form on old leaf spots or cankers. Bolls may become infected causing boll rot, rotted seeds and discolored lint. Infected bolls have round, rather than angular, lesions that initially may appear water-soaked. As the infection proceeds, bolls lesions will be sunken and dark brown or black.",
      "organic_control": null,
      "preventive_measures": null
    },
    {
      "confidence": 0,
      "crop_name": "Cotton",
      "disease": "Curl Virus",
      "crop_id": "b0f6be504c69440897464389f9374a81",
      "causes": null,
      "chemical_control": null,
      "symptoms": "The primary symptom of cotton leaf curl virus is the upward curling of leaves. Additionally, leaf veins can thicken and darken, and outgrowths (enactions) may form on the undersides of leaves, typically in the shape of leaves. Flowers may stay closed and then drop along with the bolls. If plants are infected early in the season, their growth will be stunted and yield will be reduced significantly.",
      "organic_control": null,
      "preventive_measures": null
    },
    {
      "confidence": 0,
      "crop_name": "Potatoes",
      "disease": "Early Blight",
      "crop_id": "495fe21b4389460a811f7ee97e9af8c0",
      "causes": null,
      "chemical_control": null,
      "symptoms": "Symptoms of early blight occur on older foliage, stem, and fruits. \nGray to brown spots appear on leaves and gradually grow in a concentric manner around a clear center - the characteristic bullseye formation. \nThese lesions are surrounded by a bright yellow halo.\nAs the disease progresses, entire leaves may turn chlorotic and shed, leading to significant defoliation.\nWhen leaves die and fall, fruits become more vulnerable to sun scald.\nThe same type of spots with a clear center appears on stems and fruits.\nThe fruits begin to rot and may eventually fall off.",
      "organic_control": null,
      "preventive_measures": null
    }
  ];

  export default { dummyData };